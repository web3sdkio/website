...
const handler = async (req: NextApiRequest, res: NextApiResponse) => {
  ...
  try {
    ...
    const sdk = getEVMWeb3sdkioSDK(chainId as SUPPORTED_CHAIN_ID);
...
async function fetchConstructorParams(
  ...
  if (
    ...
    try {
      ...
      // contracts not released through web3sdkio
    }
    ...
    try {
      ...
      throw new Error(
        "Verifying this contract requires a release. Run `npx web3sdkio release` to create a release for this contract, then try again.",
      );
    }
...
async function fetchDeployBytecodeFromReleaseMetadata(
  ...
  if (compialierMetaUri) {
    const pubmeta = await getEVMWeb3sdkioSDK(ChainId.Polygon)
      .getPublisher()
